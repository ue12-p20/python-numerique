...
On vous redonne l'import de `matplotlib` pour `plt.imshow` afin voir vos zébrures (vous pouvez vous amuser à faire une image en RGB c'est à dire au lieu d'avoir un pixel à `1 `ou à `0` vous avez un pixel qui est une colonne de 3 `1` ou de 3 `0`).

En variantes:  
(i)  mettez une épaisseur aux rayures (pensez à np.indices, au modulo, au test)   
(ii) mettez les rayures en diagonales  
...
<!-- #endregion -->
